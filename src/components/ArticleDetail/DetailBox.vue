      ...
      <div class="article-content"><!--文章内容-->
        ...
        <div><!--点赞按钮和评论按钮-->
          <el-badge :value=this.likesCount class="item">
            <el-button style="margin-left:50rem" id="like-button" size="small" @click=like($event)>点赞</el-button>
          </el-badge>
        </div>
        <!--分割线-->
        ...
        <div id="comment-box"><!--评论区展示box-->
          <div v-for='(comment1,index) in commentList' :key="comment1.commentId" class="each-comment1" style="margin:0rem 0rem 2rem 0rem"><!--每一条一级评论-->
          <!--一级评论的布局-->
              <el-row>
                <el-col :span="20">
                  <div class="grid-content bg-purple">
                    <div class="flex-start"><!--应该将评论者的头像和昵称放在同一行-->
                      ...
                      <div class="comment-name">{{comment1.userName}}:</div><!--评论者的昵称-->
                    </div>
                  </div>
                </el-col>
                <!--回复举报按钮-->
                ...
                <el-col :span="20">
                  <div class="grid-content bg-purple">
                    <div class="flex-start">
                      ...
                      <div class="flex-start">
                        <div class="comment-name">{{comment2.userName}}</div>
                         <div style="margin:0rem 0.2rem 0rem 0.2rem;font-size:0.8rem">回复</div>
                         <div class="comment-name">{{comment2.replyUserName}}:   {{comment2.createTime}}
                        </div>
                      </div>
        ...
        <div><!--加载更多一级评论-->
          <el-link type="primary" @click=more_comment1()>{{loadmessage}}</el-link>
        </div>
      </div>
      <!--点赞和评论-->
  </div>
...
export default {
  ...
  methods: {
       ...
       this.$http.get("http://101.43.198.16:10000/article/getInfo?artId="+this.articleId,{headers:{'Authorization':this.$store.state.token}}).then((res)=>{
                    ...
                    if(res.data.code==200){
                      ...
                      if(this.commentsCount==0){
                            this.loadmessage="该文章暂时没有评论，快来抢占前排吧！"
                          }
                      //初始化点赞按钮颜色
    ...
    //加载更多评论
    more_comment1 () {
        let new_num=this.commentList.length
        console.log(this.commentList.length)
         ...
         params: {
              ...
              console.log(this.commentList.length)
              if(new_num==this.commentList.length){
                this.loadmessage="没有更多评论了"
              }
            })
      }
